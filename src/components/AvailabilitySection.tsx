...
// Sample data - in production would come from a backend
const getDateStatus = (hall: HallType, date: Date): DateStatus => {
  const day = date.getDay();
  if (day === 1 || day === 2) return "closed"; // Mon, Tue closed
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="flex border border-border rounded-sm overflow-hidden">
              {(["grand", "boutique"] as HallType[]).map((h) => (
                <button
                  key={h}
                  onClick={() => setHall(h)}
                  className={`px-8 py-3 font-sans text-xs tracking-[0.2em] uppercase transition-all duration-300 ${
                    hall === h
                      ? "bg-gold text-primary-foreground"
                      : "text-muted-foreground hover:text-cream"
                  }`}
                >
                  {h === "grand" ? "Duża Sala" : "Mała Sala"}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-card p-6 md:p-10 border border-border">
            ...
            <div className="grid grid-cols-7 gap-2 mb-2">
              {DAYS_PL.map((d) => (
                <div key={d} className="text-center font-sans text-[10px] tracking-[0.2em] uppercase text-muted-foreground py-2">
                  {d}
                </div>
              ))}
            </div>
              ...
              {days.map((date, i) => {
                ...
                return (
                  <button
                    ...
                    className={`aspect-square flex items-center justify-center font-sans text-sm rounded-sm transition-all duration-200 ${statusColors[status]}`}
                  >
                    {date.getDate()}
                  </button>
                );
            ...
            <div className="flex flex-wrap justify-center gap-6 mt-8 pt-6 border-t border-border">
              <div className="flex items-center gap-2">
                ...
                <span className="font-sans text-xs text-muted-foreground">Dostępna</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm bg-secondary" />
                <span className="font-sans text-xs text-muted-foreground">Zarezerwowana</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm bg-muted/30" />
                <span className="font-sans text-xs text-muted-foreground">Zamknięte</span>
              </div>
            </div>
          </div>
        </div>
